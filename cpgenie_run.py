...
def model(X_train, Y_train, X_test, Y_test):
    ...
    model = Sequential()
    model.add(Convolution2D(128, 1, 5, padding='same', input_shape=(4, 1, datasize),activation='relu',W_constraint=maxnorm(W_maxnorm)))
    model.add(MaxPooling2D(pool_size=(1, 5),strides=(1,3)))
    model.add(Convolution2D(256,1,5, padding='same',activation='relu',W_constraint=maxnorm(W_maxnorm)))
    model.add(MaxPooling2D(pool_size=(1, 5),strides=(1,3)))
    model.add(Convolution2D(512,1,5, padding='same',activation='relu',W_constraint=maxnorm(W_maxnorm)))
    model.add(MaxPooling2D(pool_size=(1, 5),strides=(1,3)))
    model.add(Flatten())
...
model.compile(loss='binary_crossentropy', optimizer=myoptimizer,metrics=['accuracy'])
model.fit(x_train, y_train, batch_size=100,validation_split=0.1)
score, acc = model.evaluate(x_test,y_test)
