...
def input_maker(methylations, datasize, window_size, organism_name, from_file, context, half_w):
...
def profiler(cnfg, methylations, context, datasize, window_size=20, from_file=False, threshold=0.5):
    ...
    X_methylated, Y_methylated = input_maker(methylated, int(datasize/2), window_size, organism_name, from_file, context, half_w)
    X_unmethylated, Y_unmethylated = input_maker(unmethylated, int(datasize/2), window_size, organism_name, from_file, context, half_w)
...
def experiments(config_list, context_list, dataset_size=50000, window_size=20, coverage_threshold=10):
    ...
    for cnfg in config_list:
        ...
        for context in context_list:
            ...
            for test_context in context_list:
                if test_context != context:
                    test_methylations, test_num_to_chr_dic = pg.get_methylations(cnfg, '', coverage_threshold)
                    X_test, Y_test = profiler(cnfg, test_methylations, test_context, test_dataset_size, window_size=window_size)
